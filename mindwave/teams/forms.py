...
class Edit_team_form(forms.ModelForm):
    team_name = forms.CharField(max_length=100)
    members = forms.ModelMultipleChoiceField(queryset=Profile.objects.all(), widget=forms.CheckboxSelectMultiple)
    ...
    def __init__(self, *args, **kwargs):
        super(Edit_team_form, self).__init__(*args, **kwargs)
        self.helper = FormHelper()
        self.helper.add_input(Submit('submit', 'Update Team'))
        
    def save(self, commit=True):
        team = super(Edit_team_form, self).save(commit=False)
        if commit:
            team.save()
            self.save_m2m() 
        return team